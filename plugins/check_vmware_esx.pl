#!/appcentreon/logiciel/list/perl/bin/perl -w
#
...
#use lib "/var/tmp/vmware-vsphere-cli-distrib/lib/VMware/share";
use lib "/appcentreon/logiciel/list/vmware/";
use strict;
use warnings;
use File::Basename;
use HTTP::Date;
...
# Own modules
#use lib "modules";
use lib "/appcentreon/logiciel/list/vmware/modules";
use help;
...
our $readWarning;                                  # Warning threshold.
our $readCritical;                                 # Critical threshold.
our $writeWarning;                                  # Warning threshold.
our $writeCritical;                                 # Critical threshold.
#Pour baies HUS (low, hors prod)

our $isHUS = 0;					#baie hus flag
our $husreadWarning;                                  # Warning threshold.
our $husreadCritical;                                 # Critical threshold.
our $huswriteWarning;                                  # Warning threshold.
our $huswriteCritical;                                 # Critical threshold.

#Pour baies HUSVM (low, prod)

our $isHUSVM = 0;					#baie hus flag
our $husvmreadWarning;                                  # Warning threshold.
our $husvmreadCritical;                                 # Critical threshold.
our $husvmwriteWarning;                                  # Warning threshold.
our $husvmwriteCritical;                                 # Critical threshold.
...
my  $plugin_cache="/tmp/"; 			# Directory for caching plaugin data. Good idea to use a tmpfs
...
GetOptions
	("h:s" => \$help,                "help:s"           => \$help,
	 ...
	 "warning=s"        => \$warning,
	 "rw=s" => \$readWarning,        "read_warning=s"   => \$readWarning,
	 "ww=s" => \$writeWarning,       "write_warning=s"  => \$writeWarning,
	 "rc=s" => \$readCritical,       "read_Critical=s"  => \$readCritical,
	 "wc=s" => \$writeCritical,      "write_Critical=s" => \$writeCritical,
	 "hrw=s" => \$husreadWarning,        "husread_warning=s"   => \$husreadWarning,
	 "hww=s" => \$huswriteWarning,       "huswrite_warning=s"  => \$huswriteWarning,
	 "hrc=s" => \$husreadCritical,       "husread_Critical=s"  => \$husreadCritical,
	 "hwc=s" => \$huswriteCritical,      "huswrite_Critical=s" => \$huswriteCritical,
	 "hvrw=s" => \$husvmreadWarning,        "husvmread_warning=s"   => \$husvmreadWarning,
	 "hvww=s" => \$husvmwriteWarning,       "husvmwrite_warning=s"  => \$husvmwriteWarning,
	 "hvrc=s" => \$husvmreadCritical,       "husvmread_Critical=s"  => \$husvmreadCritical,
	 "hvwc=s" => \$husvmwriteCritical,      "husvmwrite_Critical=s" => \$husvmwriteCritical,
	 "wmem=s" => \$memWarning,       "mem_warning=s"    => \$memWarning,
	 "cmem=s" => \$memCritical,      "mem_critical=s"   => \$memCritical,
	 "wcpu=s" => \$cpuWarning,       "cpu_warning=s"    => \$cpuWarning,
	 "ccpu=s" => \$cpuCritical,      "cpu_critical=s"   => \$cpuCritical,
...
sub check_against_threshold_read
    {
	if ($isHUS)
	{
		$warning=$husreadWarning;
		$critical=$husreadCritical;
	}
	elsif ($isHUSVM)
	{
		$warning=$husvmreadWarning;
		$critical=$husvmreadCritical;
	}
	else
	{
		$warning=$readWarning;
	...
	return check_against_threshold(@_);
    }



sub check_against_threshold_write
    {
	if ($isHUS)
        {
		$warning=$huswriteWarning;
	        $critical=$huswriteCritical;
	}
	elsif ($isHUSVM)
	{
		$warning=$husvmwriteWarning;
		$critical=$husvmwriteCritical;
	}
	else
	{
		$warning=$writeWarning;
		$critical=$writeCritical;
	}
	return check_against_threshold(@_);
    }
sub check_against_threshold_cpu
    {
        $warning=$cpuWarning;
        $critical=$cpuCritical;
        return check_against_threshold(@_);
    }
sub check_against_threshold_mem
    {
        $warning=$memWarning;
