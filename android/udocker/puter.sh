...
if [ -n "$1" ]; then
  unset cmd
  cmd="$*"
  udocker_run --entrypoint "sh -c" -p "$PORT:4100" -e NODE_OPTIONS=--no-node-snapshot -v "$DATA_DIR/config:/etc/puter" -v "$DATA_DIR/data:/var/puter" "$CONTAINER_NAME" "$cmd"
else
  udocker_run --entrypoint "sh -c" -p "$PORT:4100" -e NODE_OPTIONS=--no-node-snapshot -v "$DATA_DIR/config:/etc/puter" -v "$DATA_DIR/data:/var/puter" "$CONTAINER_NAME" ' \
